...
class SubscriptionMgr(object):
                ...
                async with websockets.connect(SubscriptionMgr.WEB_SOCKET_URI, ssl=self.ssl_context) as websocket:
                    ...
                    while True:
                        ...
                        if response['type'] == "SUBSCRIPTIONS":
                            logger.info(f"Subscription confirmed for channels [" + ",".join(
                                [channel["name"] for channel in response["channels"]]) + "]")

                        # subscription negative response
                        elif response['type'] == "ERROR":
                            raise Exception(
                                f"Subscription error. Request [{json.dumps(subscription_message)}] Response [{json.dumps(response)}]")
    ...
    def _create_subscription_message(self) -> dict:
        return {
            "type": "SUBSCRIBE",
            "channels": [
                dict(subscription.get_channel_subscription_message(), **{"api_token": self.api_key}) for subscription in
                self.subscriptions
            ]
        }
...
class CandlesticksSubscription(Subscription):
    def __init__(self, subscription_params: List[CandlesticksSubscriptionParams],
                 callbacks: List[Callable[[dict], Any]] = None):
        super().__init__(callbacks)
